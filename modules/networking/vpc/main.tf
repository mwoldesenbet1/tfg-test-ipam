...
# Create firewall subnets in us-west-2 VPC
resource "aws_subnet" "firewall_subnet_west" {
  count = 2
  provider = aws.delegated_account_us-west-2
  
  vpc_id = aws_vpc.vpc_west.id
  
  # Allocate dedicated subnet ranges for firewall
  cidr_block = cidrsubnet(aws_vpc.vpc_west.cidr_block, 3, count.index + 2)
  
  availability_zone = "${var.aws_regions[0]}${count.index == 0 ? "a" : "b"}"
  
  tags = {
    Name = "firewall-subnet-${count.index + 1}-${var.aws_regions[0]}"
    Environment = "Production"
    Purpose = "NetworkFirewall"
  }
}

# Create firewall subnets in us-east-1 VPC
resource "aws_subnet" "firewall_subnet_east" {
  count = 2
  provider = aws.delegated_account_us-east-1
  ...
  # Allocate dedicated subnet ranges for firewall
  cidr_block = cidrsubnet(aws_vpc.vpc_east.cidr_block, 3, count.index + 2)
  
  availability_zone = "${var.aws_regions[1]}${count.index == 0 ? "a" : "b"}"
  
  tags = {
    Name = "firewall-subnet-${count.index + 1}-${var.aws_regions[1]}"
    Environment = "NonProduction"
    Purpose = "NetworkFirewall"
  }
}
